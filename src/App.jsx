...
export default function App() {
  ...
  const persistSettings = useCallback(() => {
    try {
      localStorage.setItem(STORAGE_SETTINGS, JSON.stringify({
        ...
        brightness, subtitleSize, subtitleBottom, volume, cardFontSize,
      }));
    } catch {}
  }, [cardsRatio, cardsRatioLandscape, playbackRate, repeatOn, repeatCount, showEnglish, showPersian, brightness, subtitleSize, subtitleBottom, volume, cardFontSize]);
  ...
  const playCenteredCard = useCallback(() => {
    ...
    cardElements.forEach((el) => {
      ...
      if (distance < closestDistance) {
        ...
        closestIndex = Number(el.getAttribute("data-card"));
      }
    });
  ...
  const refreshAudioTracks = useCallback(() => {
    ...
    setSelectedAudioId(enabled ? enabled.id : (list[0]?.id ?? null));
  }, []);
  ...
  const handleVideoLoaded = () => {
    ...
    if (tracks) {
      ...
      tracks.onremovetrack = refreshAudioTracks;
      tracks.onchange = refreshAudioTracks;
    }
    ...
    <div dir="rtl" className="movie-pluss" style={{ fontFamily: "Vazirmatn, sans-serif", color: COLORS.text }}>
      <style>{`
        * { box-sizing: border-box; }
        html, body { margin: 0; padding: 0; background: ${COLORS.bg}; color: ${COLORS.text}; font-family: 'Vazirmatn', sans-serif; }
        ...
        @media (max-width: 767px) and (orientation: portrait) {
          ...
          .cards-container { scroll-snap-type: x mandatory; }
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 23, fontWeight: 900, color: COLORS.text }}>فیلم پلاس</div>
                  ...
                  >
                      ...
                      <div className="settings-popup" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{ marginTop: 10, marginBottom: 12, color: COLORS.text, fontSize: 11 }}>
                          ...
                          {audioTracks.length > 1 ? (
                            ...
                            >
                              {audioTracks.map((t) => (
                                <option key={t.id} value={t.id} style={{ background: COLORS.card }}>
                    ...
                    {activeCue && (
                      <div style={{ position: "absolute", right: 0, bottom: subtitleBottom, left: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, padding: "0 18px", pointerEvents: "none", zIndex: 1000 }}>
                        {showEnglish && activeCue.en && <div style={{ maxWidth: "92%", padding: "5px 12px", borderRadius: 6, background: "rgba(0,0,0,.78)", color: COLORS.yellow, fontSize: 17 * (subtitleSize / 100), fontWeight: 700, textAlign: "center", direction: "ltr", pointerEvents: "auto" }}>{renderEnglish(activeCue.en, "overlay")}</div>}
                        {showPersian && activeCue.fa && <div style={{ maxWidth: "92%", padding: "5px 12px", borderRadius: 6, background: "rgba(0,0,0,.78)", color: COLORS.teal, fontSize: 17 * (subtitleSize / 100), fontWeight: 700, textAlign: "center", pointerEvents: "auto" }}>{activeCue.fa}</div>}
                      </div>
                    )}
                  </div>
                </div>
